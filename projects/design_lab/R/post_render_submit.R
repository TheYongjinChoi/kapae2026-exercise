...
find_config <- function(project_root) {
  # First choice is the hidden support folder created by the standalone Step 1.
  # The older repository locations remain as fallbacks for instructor/repo use.
  candidates <- c(
    file.path(project_root, ".kapae", "config.json"),
    file.path(project_root, "design_lab", "config.json"),
    file.path(project_root, "projects", "design_lab", "config.json")
  )

  existing <- candidates[file.exists(candidates)]
  existing[1] %||% NA_character_
}

resolve_output_path <- function(project_root, x) {
  is_absolute <- grepl("^/", x) || grepl("^[A-Za-z]:[/\\\\]", x)

  if (is_absolute) {
    normalizePath(x, winslash = "/", mustWork = FALSE)
  } else {
    normalizePath(
      file.path(project_root, x),
      winslash = "/",
      mustWork = FALSE
    )
  }
...
if (is.na(config_path)) {
  message(
    "HTML 제출 건너뜀: .kapae/config.json 또는 수업 config.json을 찾지 못했습니다."
  )
  quit(save = "no", status = 0)
}
...
for (output_rel in outputs) {
  html_path <- resolve_output_path(project_root, output_rel)
  ...
  if (!file.exists(qmd_path)) {
    message(
      "HTML 제출 건너뜀: 대응하는 QMD를 찾지 못했습니다: ",
      basename(html_path)
    )
    next
  }
  ...
  if (is.na(student_id) || is.na(method)) {
    message(
      "HTML 제출 건너뜀: student-id 또는 design-method를 읽지 못했습니다."
    )
    next
  }
  ...
  tryCatch(
    ...
    error = function(e) {
      # Do not delete the locally rendered HTML if upload fails.
      message(
