...
set_student <- function(id) {
  ...
  if (missing(id) || !nzchar(id) || (!web && id %in% c("ID 입력", "학번입력", "abc"))) {
...
CHECKS <- list(
  ...
  "task1-1-split" = list(
    ...
    call_from = NULL,
    note = "분할은 모형을 고르기 전에 먼저 해 두는 작업입니다. 테스트 데이터를 한 번이라도 보고 나면 그 정보가 판단에 섞여 들어가 마지막에 계산하는 성능이 실제보다 좋게 나옵니다. 이 블록에서는 랜덤하게 80%를 훈련, 20%를 테스트에 배정했습니다.",
    rules = list(
  ...
  "task1-2-ols" = list(
    ...
    call_from = "ols_model",
    note = "우리에게 익숙한 선형회귀 모형으로 모형을 정의했습니다. 이 결과로 아래에서 예측값을 계산하고 손실을 평가하게 됩니다. 앞 강의에서 나눈 네 단계 가운데 손실함수와 최적화가 보이지 않는데, 생략된 것이 아니라 lm() 함수 안에 들어가 있습니다. 최소제곱법이므로 손실함수는 MSE로 이미 정해져 있고, 최적화는 정규방정식을 풀어 해를 한 번에 구하기 때문에 반복 과정이 드러나지 않습니다.",
    rules = list(
  ...
  "task1-3-predict" = list(
    ...
    call_from = NULL,
    note = "`predict()` 함수를 사용해서 예측 결과를 계산하고, 그걸 `sqrt(mean(...))`에 넣어 RMSE를 계산했습니다. 이 흐름은 워크숍 전반에 걸쳐 반복되니 잘 기억해 두세요.",
    rules = list(
  ...
  "task2-1-matrix" = list(
    need = c("x_all", "y_all", "x_train", "x_test", "y_train", "y_test",
             "model_df", "train_id"),
    call_from = NULL,
    note = "glmnet() 실행을 위해 훈련 데이터를 분할했습니다. glmnet은 공식(formula)을 받지 않으므로 데이터프레임을 숫자 행렬로 바꾸는 단계가 필요하다는 점을 기억해 두세요.",
    rules = list(
  ...
  "task2-2-lasso" = list(
    ...
    call_from = NULL,
    note = "lambda는 데이터에서 추정되는 모수가 아니라 연구자가 정해야 하는 하이퍼파라미터라는 점을 기억해 두세요. 어떤 값이 맞는지 미리 알 수 없으므로 후보를 넓게 계산해 두고 교차검증 오차가 낮은 쪽을 고릅니다. 곡선의 위쪽 축에 표시된 숫자가 각 lambda에서 살아남은 변수의 개수인데, 오른쪽으로 갈수록 벌점이 강해져 그 수가 줄어듭니다.",
    rules = list(
  ...
  "task2-3-eval" = list(
    ...
    call_from = NULL,
    note = "앞서 Task 1-3과 마찬가지로 `predict()` 함수로 예측값을 계산하고, 이어서 RMSE를 계산했습니다. 이 값을 Task 1-3의 RMSE와 비교해 보면, 정규화가 예측 성능을 얼마나 개선했는지 확인할 수 있습니다.",
    rules = list(
  ...
  "task2-4-coef" = list(
    ...
    call_from = NULL,
    note = "표에 나타나지 않는 변수는 Lasso가 계수를 정확히 0으로 만들어 모형에서 제외한 변수입니다. 표준화 계수를 함께 계산한 것은 원래 계수의 크기가 변수의 단위에 좌우되기 때문입니다. 소득처럼 값이 큰 변수는 계수가 작게 나오고 이진변수는 크게 나오므로, 그대로 비교하면 어느 변수가 더 중요한지 잘못 읽게 됩니다. 다만 여기서 상위에 오른 변수를 인과적으로 해석하면 안된다는 점에 주의하세요. Lasso는 예측에 유용한 변수를 남기는 것이지 결과를 변화시키는 변수를 찾는 것이 아니고, 서로 상관된 변수들 가운데 하나만 임의로 남기는 성질도 있습니다.",
    rules = list(
  ...
  "task3-1-ridge" = list(
    ...
    call_from = NULL,
    note = "Ridge의 계수 표에는 0이 하나도 없습니다. 제곱 벌점은 계수를 0 쪽으로 당기지만 정확히 0으로 만들지는 않기 때문이고, 이 부분이 절댓값 벌점을 쓰는 Lasso와 다른 점입니다.",
    rules = list(
